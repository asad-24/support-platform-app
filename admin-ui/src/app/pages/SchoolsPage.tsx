...
import { Archive, CheckCircle2, Edit, Eye, Plus, RefreshCw, RotateCcw, Save, Search, XCircle } from "lucide-react";
...
import type { ManagedUser, School, SchoolStatus } from "../lib/types";
    ...
    <div className="p-4 sm:p-6">
      <div className="mb-6 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Review submitted schools and manage school records.</p>
        </div>
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-5xl">
          ...
          {detailLoading && <div className="rounded-md border border-dashed p-6 text-center text-sm text-muted-foreground">Loading school...</div>}
              ...
              <Section title="Review history">
                <div className="space-y-2">
                  {(selected.reviews || []).length === 0 ? <EmptyDetail label="No reviews recorded." /> : selected.reviews?.map((review) => (
                    <div key={review.id} className="rounded-md border bg-card p-3">
  ...
  return (
    <div className="space-y-1">
      ...
      <select
        ...
        className="border-input bg-input-background focus-visible:border-ring focus-visible:ring-ring/50 h-9 w-full rounded-md border px-3 text-sm outline-none focus-visible:ring-[3px]"
      >
        {children}
      </select>
    </div>
...
function detailFields(school: School) {
  return [
    ...
    { label: "Coordinates", value: school.location?.latitude && school.location?.longitude ? `${school.location.latitude}, ${school.location.longitude}` : null },
